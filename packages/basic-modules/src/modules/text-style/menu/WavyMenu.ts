...
class WavyLineMenu implements IButtonMenu {
  protected readonly marksNeedToRemove: string[] = ['underline'] // 增加 mark 的同时，需要移除哪些 mark （互斥，不能共存的）

  readonly mark = 'wavy'

  readonly title = '波浪线'

  readonly iconSvg = WAVY_ICON
  ...
  exec(editor: IDomEditor, value: string | boolean) {
    ...
    if (!isSelectedAllText(editor)) {
      setNodeMarks(editor, { [mark]: 'underline wavy' }, marksNeedToRemove, ['PINYIN'])
    } else if (value) {
      // 已，则取消
      editor.removeMark(mark)
    } else {
      // 没有，则执行
      editor.addMark(mark, 'underline wavy')
      // 移除互斥、不能共存的 marks
